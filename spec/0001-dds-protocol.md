...
| **Related** | [Anonymity Addendum](./0001-anonymity-addendum.md), [Implementation Addendum](./0001-implementation-addendum.md) |
...
> **Two distinct privacy concerns**: DDS distinguishes between **participant anonymity** (hiding _who_ said what, addressed by per-deliberation DIDs, ZK proofs, and the [Anonymity Addendum](./0001-anonymity-addendum.md)) and **deliberation access** (restricting _who_ can see and participate, addressed by access control and encryption). These are orthogonal: a public deliberation can have anonymous participants, and a private deliberation can have identified participants. Both are valid requirements. DDS prioritizes public deliberation but must support private/restricted deliberations for practical use cases (event-only discussions, private group deliberations). See §6 for deliberation access and the [Anonymity Addendum](./0001-anonymity-addendum.md) for participant anonymity.
...
Running analysis, including clustering (PCA, Reddwarf), LLM sensemaking, and other methods, requires:
...
- **Infrastructure**: Maintaining servers to process deliberations continuously

For a single user to verify results independently, they'd need to replicate this entire pipeline. This is impractical at scale, as most users lack both the infrastructure and the expertise.
...
See [Implementation Addendum §6](./0001-implementation-addendum.md#6-result-commitment-protocol) for protocol details.
...
>
> **On deliberation access** (distinct from participant anonymity): Deliberation access restricts _who_ can participate in a deliberation. This is orthogonal to participant anonymity (hiding _who_ said what). A private deliberation can have identified participants; a public deliberation can have anonymous participants. Organizers often need frictionless deliberations (no login required) restricted to a select group of invited people. Two modes: **participation-gated** (the common case: deliberation is publicly viewable, but only invited people can submit, via a secret embedded in the share link) and **restricted** (rare: deliberation can't even be viewed by outsiders, via encryption). For restricted deliberations, AT Protocol currently only handles public data; long-term, end-to-end encryption (Signal model) is the priority, and research in this direction is already underway in the AT Protocol community. That said, this use case is marginal, as most deliberation is public. See [Implementation Addendum §7](./0001-implementation-addendum.md#7-deliberation-access) for design exploration.
>
> **On participant anonymity**: Participant anonymity, hiding _who_ said what, is often important even outside hard-anonymity scenarios. Many participants want to share opinions without them being permanently tied to their identity. DDS supports this at multiple levels: pseudonymity (one DID, not linked to real name) as the default, per-deliberation unlinkability (ephemeral `did:key`) for credential-gated or sensitive contexts, and hard anonymity (Tor, mixnets) as future work for high-threat users. See [Anonymity Addendum](./0001-anonymity-addendum.md) for detailed analysis.
